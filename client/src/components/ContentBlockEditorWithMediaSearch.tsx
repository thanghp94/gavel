...
const ContentBlockEditorWithMediaSearch = ({ 
  initialBlocks = [], 
  onSave, 
  pageTitle, 
  onPageTitleChange,
  ...
  const [mediaSearchType, setMediaSearchType] = useState<'giphy' | 'google' | 'youtube' | null>(null);
  ...
  // Open media search modal for a block
  const openMediaSearch = (blockId: string, type: 'giphy' | 'google' | 'youtube') => {
    setCurrentEditingBlockId(blockId);
    setMediaSearchType(type);
    setMediaSearchQuery('');
    setMediaSearchResults([]);
    setIsMediaSearchOpen(true);
  };

  // Real media search function using actual APIs
  const performMediaSearch = async (query: string) => {
    if (!mediaSearchType) return;
    
    try {
      const { MediaSearchService } = await import('@/lib/mediaSearch');
      const results = await MediaSearchService.search(query, mediaSearchType);
  ...
  const selectMediaItem = (item: any) => {
    if (!currentEditingBlockId || !mediaSearchType) return;
    if (mediaSearchType === 'giphy' || mediaSearchType === 'google') {
      updateBlock(currentEditingBlockId, { imageUrl: item.url, imageAlt: item.title });
    } else if (mediaSearchType === 'youtube') {
      updateBlock(currentEditingBlockId, { videoUrl: item.url, videoTitle: item.title });
    }
    setIsMediaSearchOpen(false);
    setCurrentEditingBlockId(null);
    setMediaSearchType(null);
    setMediaSearchQuery('');
    setMediaSearchResults([]);
  };
  ...
  useEffect(() => {
    if (mediaSearchQuery.length > 2) {
      performMediaSearch(mediaSearchQuery);
    } else {
      setMediaSearchResults([]);
    }
  }, [mediaSearchQuery]);
        ...
        <CardContent>
          ...
          {block.type === 'image' && (
            <div className="space-y-4">
              ...
              <div className="flex gap-2">
                <Button size="sm" variant="outline" onClick={() => openMediaSearch(block.id, 'giphy')}>
                  <Search className="h-4 w-4 mr-1" />
                  Search Giphy
                </Button>
                <Button size="sm" variant="outline" onClick={() => openMediaSearch(block.id, 'google')}>
                  <Search className="h-4 w-4 mr-1" />
          ...
          {block.type === 'video' && (
            <div className="space-y-4">
              ...
              <Button size="sm" variant="outline" onClick={() => openMediaSearch(block.id, 'youtube')}>
                <Search className="h-4 w-4 mr-1" />
                Search YouTube
              </Button>
            </div>
    ...
    <div className="space-y-6">
      ...
      <Dialog open={isMediaSearchOpen} onOpenChange={setIsMediaSearchOpen}>
        <DialogContent className="max-w-4xl max-h-[70vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>Search {mediaSearchType === 'giphy' ? 'Giphy' : mediaSearchType === 'google' ? 'Google Images' : 'YouTube'}</DialogTitle>
            <DialogDescription>
              Search and select media to insert into the content block.
            </DialogDescription>
          </DialogHeader>
          <div className="mb-4">
            <Input
...
export default ContentBlockEditorWithMediaSearch;
